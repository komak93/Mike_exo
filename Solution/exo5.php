...
if(!empty($_POST))
{
	...
	if(count($mike)< 3)// sans crochet , c'est qu'il y a qu'une seule instruction
		...
		{
			echo "Votre paramètre $mike[2] n'est pas correcte";
		}
	else
		{	
			//---------- SIMPLE ----------- //
			/* if ($mike[1] > $mike[2]) 
			{
				echo "Votre nombre" . mike[1] . " est le plus puissant";
			}
			else 
			{	
				echo "Votre nombre " . mike[2] . " est moins puissant";
			} */
			//--------- TERNAIR ----------- //
			echo (intval($mike[1]) > intval($mike[2]))?"La puissance est $mike[1]":(intval($mike[2]) > intval($mike[1]))?"La puissance est $mike[2]":"équals";
		}
}
// $val = int()"22"
?>
